...
import { isStringingState, wouldDiscardTrackerData } from "../shared/stringing.js";
...
export function createApp(repositories: Repositories, objectStore: ObjectStore): express.Express {
  ...
  app.put("/api/stringing/state", requireAuth, requireAdmin, async (request, response, next) => {
    try {
      ...
      if (wouldDiscardTrackerData(existing?.state, request.body)) throw new HttpError(409, "The tracker has been updated. Refresh this page before saving.");
...
function teamSpeakBridgeInstallScript(baseUrl: string): string {
  return [
    ...
    "  NODE_ARCH=\"x64\"",
    "else",
    "  echo \"This installer supports Apple Silicon and Intel Macs.\"",
    "  exit 1",
    "fi",
    "RUNTIME_DIR=\"$APP_DIR/runtime/node-$NODE_VERSION-darwin-$NODE_ARCH\"",
    "NODE_BIN=\"$RUNTIME_DIR/bin/node\"",
    "REPO_DIR=\"$APP_DIR/repo\"",
    "REPO_ARCHIVE_URL=\"https://github.com/dmc2468/miro-pdf-image-converter/archive/refs/heads/main.zip\"",
    ...
    "  curl -fsSL \"https://nodejs.org/dist/$NODE_VERSION/node-$NODE_VERSION-darwin-$NODE_ARCH.tar.gz\" -o \"$NODE_ARCHIVE\"",
    "  mkdir -p \"$APP_DIR/runtime\"",
    "  tar -xzf \"$NODE_ARCHIVE\" -C \"$DOWNLOAD_DIR\"",
    "  rm -rf \"$RUNTIME_DIR\"",
    "  mv \"$DOWNLOAD_DIR/node-$NODE_VERSION-darwin-$NODE_ARCH\" \"$RUNTIME_DIR\"",
    "fi",
    "echo \"Downloading Studio McLeod TeamSpeak bridge...\"",
    "REPO_ARCHIVE=\"$DOWNLOAD_DIR/bridge.zip\"",
    "REPO_EXTRACT=\"$DOWNLOAD_DIR/bridge\"",
    "curl -fsSL \"$REPO_ARCHIVE_URL\" -o \"$REPO_ARCHIVE\"",
    "mkdir -p \"$REPO_EXTRACT\"",
    "ditto -x -k \"$REPO_ARCHIVE\" \"$REPO_EXTRACT\"",
    "rm -rf \"$REPO_DIR\"",
    "mv \"$REPO_EXTRACT/miro-pdf-image-converter-main\" \"$REPO_DIR\"",
    "cd \"$REPO_DIR\"",
    "PATH=\"$RUNTIME_DIR/bin:$PATH\" corepack enable --install-directory \"$RUNTIME_DIR/bin\"",
    "PATH=\"$RUNTIME_DIR/bin:$PATH\" corepack pnpm install --frozen-lockfile",
    "STUDIO_MCLEOD_BASE_URL=\"$STUDIO_MCLEOD_BASE_URL\" PATH=\"$RUNTIME_DIR/bin:$PATH\" corepack pnpm teamspeak:bridge:install",
  ].join("\n");
}
